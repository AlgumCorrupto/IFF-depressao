...
Como vocês notaram a função ``gets()`` espera um buffer de tipo `string` como argumento. Caso seu programa queira converter uma variável tipo `string` para tipo ``int``, é utilizado a função `atoi()`, que pega como um argumento uma `string` qualquer e retorna um `int`.
...
Também existe um análogo que converte string em números `floats` -- números de ponto flutuante ou decimais, chamada `atof()`.
